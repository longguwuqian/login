...
// Load模型
int InitialFunc(PCAModel* pPCAModel, byte* pGalleryFacePCACoeff, int &nGalleryPeopleNum)
{
...
byte* LoadGalleryData(int &nGalleryPeopleNum, PCAModel* pPCAModel, char* strPeopleNumFile, char* strGalleryModel)
{
	...
	double* pGalleryFacePCACoeff = new double[pPCAModel->m_nPCADim*m_nGalleryPeopleNum];
...
int FaceRecognition(PCAModel* pPCAModel, byte* pFaceImgGray, byte* pGalleryFacePCACoeff, int nPeopleID, int nGalleryPeopleNum,double dThreshold)
{
	if(nPeopleID >= nGalleryPeopleNum)
	{
		return -1;
	}
	pInputFacePCACoeff = new double[pPCAModel->m_nPCADim];
	...
	if (curSim >= pParentDlg->dThreshold)
	{
		return 1;
	}
	else
...
void GetFaceImg(Cgt_Eye* irisPoint, byte* pGrayImg, byte* pFaceImgGray)
{
		...
		dR = sqrt(AffineMatrix[0]*AffineMatrix[0] + AffineMatrix[1]*AffineMatrix[1]);
		...
		float cosAngle = pParentDlg->m_AffineMatrix[0] / dR;
		float sinAngle = pParentDlg->m_AffineMatrix[1] / dR;
...
BOOL GetFaceParameters(byte* gray_image,int w,int h,Cgt_Rect* area,Cgt_Eye* irisPoint)
{
 ...
 if(faces.empty() == false)
 {
  ...
  int tmpSwap;
  swap(irisPoint->xleft, irisPoint->xright, tmpSwap);
  swap(irisPoint->yleft, irisPoint->yright, tmpSwap);
  //画出框到的人脸，验证调试用
